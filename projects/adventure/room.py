...
class Room:
    ...
    def get_exits_ids(self):
        ...
        exits = []
        if self.n_to is not None:
            # exits['n'] = self.n_to.id
            exits.append(self.n_to.id)
        if self.s_to is not None:
            # exits['s'] = self.s_to.id
            exits.append(self.s_to.id)
        if self.w_to is not None:
            # exits['w'] = self.w_to.id
            exits.append(self.w_to.id)
        if self.e_to is not None:
            # exits['e'] = self.e_to.id
